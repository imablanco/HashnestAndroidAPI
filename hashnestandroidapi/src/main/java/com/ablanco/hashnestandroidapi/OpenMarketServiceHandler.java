...
public class OpenMarketServiceHandler {
    ...
    public void getOpenedMarkets(NetworkResponseListener<ArrayList<OpenMarketModel>> listener){

        BaseServiceAsyncTask<ArrayList<OpenMarketModel>> task = new BaseServiceAsyncTask<ArrayList<OpenMarketModel>>(listener){
            @Override
            protected Object doInBackground(Void... params) {
                try {
                    String response = OpenMarketNetworkHandler.getOpenedMarkets();
    ...
    public void getTradingHistpry(final String marketId, final TradingOrderType tradingOrderType,NetworkResponseListener<ArrayList<TradingOrderModel>> listener){

        BaseServiceAsyncTask<ArrayList<TradingOrderModel>> task = new BaseServiceAsyncTask<ArrayList<TradingOrderModel>>(listener){
            @Override
            protected Object doInBackground(Void... params) {
                try {
                    String response = OpenMarketNetworkHandler.getTradingOrderHistory(marketId,tradingOrderType);
